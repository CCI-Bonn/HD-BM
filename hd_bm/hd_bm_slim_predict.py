...
def main():
    parser = argparse.ArgumentParser(
        description="This script will allow you to run HD-BM (slim) to predict a single case with modalities"
        " (with index):\n T1ce (0000), FLAIR (0001)"
        "The different modalities should follow nnUNet naming convention `{SOME_ID}_{ModalityID}.nii.gz`"
        "To predict all cases in a directory, please use `hd_bm_slim_predict_folder` (this one "
    ...
    input_paths, output_file = prepare_input_args_hd_bm_slim(
        inp_dir, inp_id, output_dir, output_id, skip_modality
    )

    predict_cases(
        folder_with_hd_bm_slim_parameter_files,
        [input_paths],
        [output_file],
        (0, 1, 2, 3, 4),
        False,
